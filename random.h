...
/** @defgroup test_tool Tool set of test
...
/** Random defines a tool class for generating random integer as unit test
...
class Random {
  public:
     ...
     */
    explicit Random(const int min_bound, const int max_bound,
                       const uint32_t seed)
        : gen_(seed) {
        setup(min_bound, max_bound);
    }
    ...
    explicit Random(const int nbits, const bool is_signed,
                       const uint32_t seed)
        : gen_(seed) {
        calculate_bounds(nbits, is_signed);
    }
